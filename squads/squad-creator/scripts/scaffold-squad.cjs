...
const WORKSPACE_LEVELS = new Set(['none', 'read_only', 'read_write', 'workspace_first']);
...
python3 squads/squad-creator/scripts/squad-analytics.py ${slug}
\`\`\`
...
Options:
  ...
  --workspace-level <none|read_only|read_write|workspace_first>
...
Creates files: README.md, config.yaml`);
  ...
  if (!WORKSPACE_LEVELS.has(workspaceLevel)) {
    outputError('INVALID_WORKSPACE_LEVEL', 'workspace level must be one of: none, read_only, read_write, workspace_first', {
