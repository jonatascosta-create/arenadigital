...
**Quality Standard:** AIOX Level (300+ lines, valid YAML/MD)
**Model:** `Sonnet` (interactive template architecture and placeholder strategy)
**Haiku Eligible:** NO -- structure decisions and elicitation design require reasoning
...
This task creates output templates for AIOX squads. Templates define the structure and format of artifacts that agents and tasks produce.
...
**Quality Checklist:** Execute `checklists/task-anatomy-checklist.md` at PHASE 3 (Validation) before finalizing output.
...
veto_conditions:
  - id: "VETO-TPL-001"
    condition: "Target template file already exists"
    trigger: "Before writing template output to templates/"
    block_behavior: "BLOCK overwrite unless explicit confirmation or --force is provided"

  - id: "VETO-TPL-002"
    condition: "Template ID collides with existing template_id in squad"
    trigger: "During PHASE 0 identity definition"
    block_behavior: "BLOCK creation; require unique template_id"
```
...
- Follow AIOX template standards
