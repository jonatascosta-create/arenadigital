...
TRIGGER (component created)
    ↓
[PHASE 0: STRUCTURAL COMPLETENESS] ← NEW (for squads only)
    → config.yaml exists (NOT squad.yaml)
    → entry_agent field present
    → tested field present
    → Entry agent has activation-instructions
    → Docs exist: README, CHANGELOG, ARCHITECTURE
    → Lifecycle tasks exist: create, update, delete
...
## PHASE 0: Structural Completeness (Squads Only)

**Duration:** < 3 seconds
**Blocking:** Yes
**Applies to:** Squad creation only (skip for agent, task, workflow, template)
...
structural_checks:
  - id: "SC-001"
    check: "config.yaml exists (NOT squad.yaml)"
    action: |
      test -f squads/{name}/config.yaml && ! test -f squads/{name}/squad.yaml
    on_fail: "ABORT - Must use config.yaml, not squad.yaml"

  - id: "SC-002"
    check: "entry_agent field present"
    action: "grep -q 'entry_agent:' squads/{name}/config.yaml"
    on_fail: "ABORT - Missing entry_agent in config.yaml"

  - id: "SC-003"
    check: "tested field present"
    action: "grep -q 'tested:' squads/{name}/config.yaml"
    on_fail: "ABORT - Missing tested field in config.yaml"

  - id: "SC-004"
    check: "Entry agent has activation-instructions"
    action: |
      ENTRY=$(grep 'entry_agent:' config.yaml | cut -d: -f2 | tr -d ' ')
      grep -q 'activation-instructions:' squads/{name}/agents/$ENTRY.md
    on_fail: "ABORT - Entry agent missing activation-instructions block"

  - id: "SC-005"
    check: "README.md exists"
    action: "test -f squads/{name}/README.md"
    on_fail: "ABORT - Missing README.md"

  - id: "SC-006"
    check: "CHANGELOG.md exists"
    action: "test -f squads/{name}/CHANGELOG.md"
    on_fail: "ABORT - Missing CHANGELOG.md"

  - id: "SC-007"
    check: "ARCHITECTURE.md exists"
    action: "test -f squads/{name}/ARCHITECTURE.md"
    on_fail: "ABORT - Missing ARCHITECTURE.md"

  - id: "SC-008"
    check: "update-*.md task exists"
    action: "ls squads/{name}/tasks/update-*.md 2>/dev/null | grep -q ."
    on_fail: "ABORT - Missing update task (brownfield)"

  - id: "SC-009"
    check: "delete-*.md task exists"
    action: "ls squads/{name}/tasks/delete-*.md 2>/dev/null | grep -q ."
    on_fail: "ABORT - Missing delete task (cleanup)"
```

**Why This Phase Exists:**

Erros cometidos na criação do squad "theme" (2026-02-23):
1. Usou `squad.yaml` ao invés de `config.yaml`
2. Faltou campo `entry_agent` no config
3. Faltou bloco `activation-instructions:` no entry agent
4. Listou `CHANGELOG.md` e `ARCHITECTURE.md` no README mas não criou
5. Não criou tasks de `update-*` e `delete-*`
6. Faltou campo `tested:` no metadata

Todos esses erros são **estruturais** e **determinísticos** - podem ser detectados automaticamente antes de declarar o squad como "criado".
