...
**Model:** `Sonnet` (task design and anatomy decisions require contextual reasoning)
**Haiku Eligible:** NO -- anatomy synthesis and elicitation quality gates are non-deterministic
...
This task creates a single workflow task for an AIOX squad. The key insight: **every task must follow the Task Anatomy standard (8 fields) for consistency and quality**.
